...
static struct opt_table opt_config_table[] = {
	...
	OPT_WITH_ARG("--mp-speed",
		     set_float_125_to_500, &opt_show_intval, &opt_rock_speed,
		     "Set RockMiner frequency in MHz, range 125-500"),
...
static void curses_print_status(void)
{
	...
	if(opt_rock_speed)
		{
		cg_mvwprintw(statuswin, 3, 0, " A:%.0f  R:%.0f  HW:%d  WU:%.1f/m  Freq:%.0fMHz  MP-Limited:%.2fGh/s",
			     total_diff_accepted, total_diff_rejected, hw_errors,
			     total_diff1 / total_secs * 60,opt_rock_freq,opt_rock_speed/10);
		}
	else
		{
...
static void curses_print_devstatus(struct cgpu_info *cgpu, int devno, int count)
{
	...
	if(opt_rock_speed)
		{
		cgpu->speed = (double)((cgpu->total_mhashes / dev_runtime * 1000000ull)/1000000000ull)*10;
