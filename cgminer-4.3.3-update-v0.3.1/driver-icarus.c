...
#define ROCKMINER
#define SUPPORT_RMU_DEVICE
#define ROCKMINER_PRINT_DEBUG 0
#define ROCKMINER_READ_SIZE 8
#define	NONCE_TEST
...
{
	int i;
		//if(opt_rock_freq<200||(opt_rock_freq>360))
		if(opt_rock_freq<rmdev[dev_id].min_frq||(opt_rock_freq>rmdev[dev_id].max_frq))
			{
#ifdef ROCKMINER_PRINT_DEBUG
			if(opt_rock_log)
			applog(LOG_WARNING, "RMU%d Reset Freq ,FREQ(%.0fMHz<%.0fMHz<%.0fMHz) ",dev_id,rmdev[dev_id].min_frq,opt_rock_freq,rmdev[dev_id].max_frq);
#endif
...
#ifdef SUPPORT_RMU_DEVICE
	const char golden_ob[] =
		"4679ba4ec99876bf4bfe086082b40025"
		"4df6c356451471139a3afa71e48f544a"
		"00000000000000000000000000000000"
		"aa1ff05587320b1a1426674f2fa722ce";
unsigned char nonce_bin[ROCKMINER_READ_SIZE];
#else
	const char golden_ob[] =
		"4679ba4ec99876bf4bfe086082b40025"
		"4df6c356451471139a3afa71e48f544a"
...
//#endif
	...
	switch (info->ident) {
		...
		case IDENT_RMU:
			...
			#ifdef SUPPORT_RMU_DEVICE
			if(info->ident == IDENT_AMU)
				{
				icarus->usbdev->ident = info->ident = IDENT_RMU;
			info->nonce_size = ROCKMINER_READ_SIZE;
			icarus->drv->name = "RMU";
				}
			#endif
			break;
...
if(info->ident == IDENT_RMU)
	{
		...
		if((nonce_bin[NONCE_CHIP_NO_OFFSET]&RM_PRODUCT_MASK)==RM_PRODUCT_T1)	
		{
			...
			rmdev[dev_id].max_frq = 360;
		}
		else if((nonce_bin[NONCE_CHIP_NO_OFFSET]&RM_PRODUCT_MASK)==RM_PRODUCT_TEST)	
		{
			rmdev[dev_id].product_id = ROCKMINER_TEST;
...
#if 0
		if((opt_rock_freq<rmdev[dev_id].min_frq||(opt_rock_freq>rmdev[dev_id].max_frq))||((rmdev[dev_id].chip[chip_no].freq>(rmdev[dev_id].max_frq/10-1))||(rmdev[dev_id].chip[chip_no].freq<(rmdev[dev_id].min_frq/10-1))))
		{
#ifdef ROCKMINER_PRINT_DEBUG
			if(opt_rock_log)
			applog(LOG_WARNING, "RMU%d Reset Freq ,FREQ(%.0fMHz<%.0fMHz<%.0fMHz) ",dev_id,rmdev[dev_id].min_frq,opt_rock_freq,rmdev[dev_id].max_frq);
#endif
						opt_rock_freq =rmdev[dev_id].def_frq;
		rmdev[dev_id].chip[chip_no].freq =( opt_rock_freq/10 - 1);
		}
		#endif
		if(opt_rock_speed)
		{
			if(icarus->speed>opt_rock_speed)
				speed_mode = SPEED_HIGH;
			else 
				speed_mode = SPEED_LOW;
			workdata.unused[ICARUS_UNUSED_SIZE - 3] =(speed_mode<<6)|(rmdev[dev_id].chip[chip_no].freq&0x3f); //icarus->freq/10 - 1; ; &0xc0
		}
		else
		{
			speed_mode = SPEED_AUTO;
			workdata.unused[ICARUS_UNUSED_SIZE - 3] =rmdev[dev_id].chip[chip_no].freq&0x3f; //icarus->freq/10 - 1; ; &0xc0
		}
	...
	if(info->ident == IDENT_RMU)
{
	for (chip_no = 0; chip_no < rmdev[dev_id].chip_max; chip_no++)
	{
		...
		{
		    #ifdef ROCKMINER_PRINT_DEBUG// NONCE_TEST
			if(opt_rock_log)
...
#ifdef NONCE_TEST// ROCKMINER_PRINT_DEBUG
		if(opt_rock_log||opt_rock_debug)
			{
			if(rmdev[dev_id].chip_max == 4)
		applog(LOG_WARNING, "RMU%d Freq: %d0MHz/ %d0MHz/ %d0MHz/ %d0MHz ",dev_id,rmdev[dev_id].chip[0].freq+1,rmdev[dev_id].chip[1].freq+1,rmdev[dev_id].chip[2].freq+1,rmdev[dev_id].chip[3].freq+1);
			else if(rmdev[dev_id].chip_max == 12)
				applog(LOG_WARNING, "RMU%d:%d0 %d0 %d0 %d0-%d0 %d0 %d0 %d0-%d0 %d0 %d0 %d0",dev_id,rmdev[dev_id].chip[0].freq+1,rmdev[dev_id].chip[1].freq+1,rmdev[dev_id].chip[2].freq+1,rmdev[dev_id].chip[3].freq+1,rmdev[dev_id].chip[4].freq+1,rmdev[dev_id].chip[5].freq+1,rmdev[dev_id].chip[6].freq+1,rmdev[dev_id].chip[7].freq+1,rmdev[dev_id].chip[8].freq+1,rmdev[dev_id].chip[9].freq+1,rmdev[dev_id].chip[10].freq+1,rmdev[dev_id].chip[11].freq+1);
			}
#endif
...
{
if(opt_rock_speed)
			applog(LOG_WARNING, "RMU%d Chip[%d] nonce=%d/%d/%d,freq:%d0MHz,%0.1fGhs",dev_id,i,rmdev[dev_id].chip[i].error_cnt,chip_nonce_conunts[i],chip_work_conunts[i],rmdev[dev_id].chip[i].freq+1,icarus->speed/10);
else
			applog(LOG_WARNING, "RMU%d Chip[%d] nonce=%d/%d/%d,freq:%d0MHz ",dev_id,i,rmdev[dev_id].chip[i].error_cnt,chip_nonce_conunts[i],chip_work_conunts[i],rmdev[dev_id].chip[i].freq+1);
}
			...
			if(rmdev[dev_id].chip[i].error_cnt>=12)
				{
				if(rmdev[dev_id].chip[i].freq>(rmdev[dev_id].min_frq/10 - 1))	//200Mhz
					{
					rmdev[dev_id].chip[i].freq --;
#ifdef  NONCE_TEST
				if(opt_rock_log)
				applog(LOG_WARNING, "RMU%d Chip[%d]=%d SUB FREQ TO %d0MHz,DEF(%.0fMHz) ",dev_id,i,rmdev[dev_id].chip[i].error_cnt,rmdev[dev_id].chip[i].freq+1,opt_rock_freq);
#endif
					}
				}
			else if(rmdev[dev_id].chip[i].error_cnt<=1)
				{
				if(rmdev[dev_id].chip[i].freq<(opt_rock_freq/10-1))
					{
					rmdev[dev_id].chip[i].freq ++;
#ifdef  NONCE_TEST
				if(opt_rock_log)
				applog(LOG_WARNING, "RMU%d Chip[%d]=%d ADD FREQ TO %d0MHz,DEF(%.0MHz) ",dev_id,i,rmdev[dev_id].chip[i].error_cnt,rmdev[dev_id].chip[i].freq+1,opt_rock_freq);
#endif
					}
				}
			else if(chip_nonce_conunts[i]==0)
				{
				if(rmdev[dev_id].chip[i].freq>(rmdev[dev_id].min_frq/10 - 1))	//200Mhz
				{
				rmdev[dev_id].chip[i].freq --;
#ifdef  NONCE_TEST
			if(opt_rock_log)
			applog(LOG_WARNING, "RMU%d Chip[%d]=%d SUB FREQ TO %d0MHz,No nonce",dev_id,i,rmdev[dev_id].chip[i].error_cnt,rmdev[dev_id].chip[i].freq+1,opt_rock_freq);
#endif
				}
